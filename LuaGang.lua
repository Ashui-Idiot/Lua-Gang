...
client:on('messageCreate', function(message)
	if message.content == ';update' then
		if state >= 1 then
			print('state check done')
			message.channel:send('```diff\nUpdates\n--Changed ;state command so it cant be spammed\n--Added on off commands for the dev\n--Added the update command, fixed it\n--Added hug command, 1/100 chance he hates you.```')
		end
	end
end)
